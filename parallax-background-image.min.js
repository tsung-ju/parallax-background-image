(function(a){'use strict';function b(O,Q=document){return'string'==typeof O?Q.querySelector(O):O}function f(O,Q=document){return O instanceof Element?[O]:Array.isArray(O)?O:('string'==typeof O&&(O=Q.querySelectorAll(O)),Array.prototype.slice.call(O))}function g(O){return'function'==typeof O?O:()=>O}function h(O){const Q=g(O);return R=>new Promise(S=>S(Q(R)))}function j(O,Q){return h(Q)(O).then(k)}function k(O){return new Promise((Q,R)=>{const S=document.createElement('img');S.onload=T=>{Q(T.target)},S.onerror=T=>{R(T.error)},S.src=O})}function l(O){return O.match(/url\(['"]?(.*?)['"]?\)/)[1]}function n(){const O=o();return document.head.insertBefore(O,document.head.firstElementChild),O.sheet}function o(){const O=document.createElement('style');return O.appendChild(document.createTextNode('')),O}function p(O,Q=null){return(R,S)=>{Q=Q||R;const T=O(R,S);return new B(T,Q)}}function q(O){return Object.assign({},F,O)}function s(O,Q){const R=1/O.velocityScale,S=O.viewport;return{scale:R,translateX:O.left*(R-1),translateY:((S.height-Q.height)*R-(S.height-O.height))/2,translateZ:S.perspective*(1-R)}}function t(O,Q){const R=O.viewport,S=R.top+R.height/2,T=O.top+O.height/2,U=O.top+Q.height/2;return{scale:1,translateX:0,translateY:(T-S)*(O.velocityScale-1)-(U-T),translateZ:0}}function v(){return-1!==navigator.userAgent.indexOf('Chrome/')}const w='data-parallax-element',x=O=>{const Q=window.getComputedStyle(O);return l(Q.backgroundImage)};var y=function(O,Q,R,S){var V,T=arguments.length,U=3>T?Q:null===S?S=Object.getOwnPropertyDescriptor(Q,R):S;if('object'==typeof Reflect&&'function'==typeof Reflect.decorate)U=Reflect.decorate(O,Q,R,S);else for(var W=O.length-1;0<=W;W--)(V=O[W])&&(U=(3>T?V(U):3<T?V(Q,R,U):V(Q,R))||U);return 3<T&&U&&Object.defineProperty(Q,R,U),U};class z{constructor(O,Q,R){this.style=O,this.width=Q,this.height=R,Object.assign(O,{position:'absolute',left:'0',top:'0',transformOrigin:'0 0 0',pointerEvents:'none'})}updateTransform(O){this.style.transform=`
    ...
    `,0)})();class N{constructor(O,Q=!v(),R=1e3){Q&&(R=0),this.useFallback=Q,this.viewport=new J(b(O),R)}add(O,Q={}){const R=q(Q);if(0>R.velocityScale)throw new RangeError('velocityScale must be positive');return f(O,this.viewport.element).filter(S=>!S.hasAttribute(w)).map(S=>this.addElement(S,R))}addElement(O,Q){return M(this,void 0,void 0,function*(){const R=yield j(O,Q.backgroundImage),S=new L(O,this.viewport,Q.velocityScale),T=Q.createBackground(S,R),U=this.useFallback?t:s;a.autorun(()=>{T.updateTransform(U(S,T))})})}}N.getCSSBackgroundImage=x,N.pesudoBefore=C,N.insertImg=(O,Q)=>{const R=document.createElement('img');return R.height=Q.naturalHeight,R.width=Q.naturalWidth,R.src=Q.src,O.element.insertBefore(R,O.element.firstElementChild),new z(R.style,Q.naturalWidth,Q.naturalHeight)},N.coverElement=p,window.Parallax=N})(mobx);
